...
export function getTestExecutionSnapshot(
  baseRepoSlug: string,
  submittedFiles: SubmittedFile[],
  overlaySlug?: string
): { tree: FSTree; overlayPaths: string[] } {
  if (!baseRepoSlug) {
    throw new Error('baseRepoSlug is required for test execution snapshot');
  }

  const basePath = path.join(SNAPSHOTS_ROOT, "base", baseRepoSlug);
  const overlayPath = overlaySlug ? path.join(SNAPSHOTS_ROOT, "overlays", overlaySlug) : null;
  const hiddenTestPath = overlaySlug ? path.join(SNAPSHOTS_ROOT, "hidden-tests", overlaySlug) : null;
  ...
  if (hiddenTestPath && fs.existsSync(hiddenTestPath)) {
    applyOverlay(tree, hiddenTestPath);
  }
