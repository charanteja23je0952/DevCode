...
// TODO: Implement search, sorting, and column projection.
// `filteredTasks`, `sortedTasks`, and `filteredColumns` are consumed below.
...
import { getDragUpdate } from "../utils/dragDropLogic";
import { buildNewTaskPayload } from "../utils/taskCreationLogic";
...
const TrelloBoard = () => {
  ...
  const onDragEnd = async (result) => {
    const updatedTask = getDragUpdate(result, tasks);
    if (!updatedTask) return;

    try {
      await axios.put(`${API_URL}/api/v1/tasks/reorder`, {
        tasks: [{ id: updatedTask._id, status: updatedTask.status }],
      });
  ...
  const addNewTask = async () => {
    const newTask = buildNewTaskPayload();
